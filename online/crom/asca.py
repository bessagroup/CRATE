...
class ASCA:
    ...
    def solve_equilibrium_problem(self, crve, material_state, mac_load,
        ...
        if is_vtk_output:
            ...
            vtk_output.write_vtk_file_time_step(0, self._strain_formulation,
                self._problem_type, crve, material_state,
                vtk_vars=vtk_data['vtk_vars'],
        ...
        while True:
            ...
            if not self._is_farfield_formulation:
                # Initialize strain tensor where each component is defined as:
                # (a) Current applied macroscale strain (if prescribed
                #     macroscale strain component)
                # (b) Current applied homogenized strain (if non-prescribed
                #     macroscale strain component)
                ...
                # Initialize stress tensor where each component is defined as:
                # (a) Current applied macroscale stress (if prescribed
                #     macroscale stress component)
                # (b) Current applied homogenized stress (if non-prescribed
                #     macroscale stress component)
            ...
            while True:
                ...
                while True:
                    ...
                    type(self)._display_nr_iter_data(
                        self._is_farfield_formulation, mode='iter',
                        nr_iter=nr_iter,
                    ...
                    if self._is_farfield_formulation:
                        ...
                        if n_presc_stress > 0:
                            applied_mix_strain_mf[presc_stress_idxs] = \
                                applied_mix_strain_mf[presc_stress_idxs] \
                                + d_iter[crve.get_n_total_clusters()\
                                         *len(comp_order):]
                #
    ...
    def _build_residual(self, crve, material_state, presc_strain_idxs,
        ...
        **Finite strains (total equilibrium formulation, \
        ...
        applied_mix_stress_mf : numpy.ndarray (1d), default=None
            Incremental stress tensor (matricial form) that contains prescribed
            stress components and (non-prescribed) homogenized stress
            components.
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            # Compute clusters equilibrium residuals
            residual[0:n_total_clusters*len(comp_order)] = np.subtract(
                np.add(global_strain_mf, np.matmul(global_cit_mf,
                                                   global_pol_stress_mf)),
                numpy.matlib.repmat(applied_mix_strain_mf, 1,
                                    n_total_clusters))
            # Compute additional residual if there are prescribed loading
            # stress components
            if n_presc_stress > 0:
    ...
    def _build_jacobian(self, crve, material_state, presc_strain_idxs,
        ...
        *Finite strains:*
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            jacobian = np.zeros(2*(n_total_clusters*len(comp_order)
                                   + n_presc_stress,))
            # Compute Jacobian matrix component solely related with the
            # clusters equilibrium residuals
            i_init = 0
            i_end = n_total_clusters*len(comp_order)
            j_init = 0
            j_end = n_total_clusters*len(comp_order)
            if self._strain_formulation == 'infinitesimal':
                ...
                jacobian[i_init:i_end, j_init:j_end] = \
                    scipy.linalg.block_diag(*(n_total_clusters*[foid_mf,])) \
                    + global_cit_diff_tangent_mf
            # Compute Jacobian matrix components arising due to the prescribed
            # loading stress components
            if n_presc_stress > 0:
                # Compute Jacobian matrix component related with the clusters
                # equilibrium residuals
                i_init = 0
                i_end = n_total_clusters*len(comp_order)
                j_init = n_total_clusters*len(comp_order)
                ...
                if self._strain_formulation == 'infinitesimal':
                    ...
                    jacobian[i_init:i_end, j_init:j_end] = \
                        numpy.matlib.repmat(-1.0*foid_mf[:,presc_stress_idxs],
                                            n_total_clusters, 1)
                # Compute Jacobian matrix component related with the prescribed
                # loading stress components
                jclst = 0
                for mat_phase in material_phases:
                    for cluster in phase_clusters[mat_phase]:
    ...
    def _check_convergence(self, crve, material_state, presc_strain_idxs,
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            error_1 = \
                np.linalg.norm(residual[0:n_total_clusters*len(comp_order)]) \
                /strain_norm_factor
            # Compute error associated with the stress homogenization
            ...
            if n_presc_stress == 0:
                ...
                is_converged = (error_1 < self._conv_tol) \
                    and (error_2 < self._conv_tol)
            # Compute the normalized error associated with the homogenized
            # strain tensor (relative to the prescribed loading strain tensor)
    ...
    def _crve_effective_tangent_modulus(self, crve, material_state,
            global_cit_diff_tangent_mf, global_strain_mf=None,
            farfield_strain_mf=None):
        """CRVE tangent modulus and clusters strain concentration tensors.
    ...
    # -------------------------------------------------------------------------
    @staticmethod
    def _display_nr_iter_data(is_farfield_formulation, mode='init',
                              nr_iter=None, nr_iter_time=None, errors=[]):
        """Display Newton-Raphson iteration data.
    ...
    def _set_output_files(self, output_dir, crve, problem_name='problem',
        ...
        if is_ref_material_output:
            ...
            ref_mat_output = RefMatOutput(
                ...
                self._self_consistent_scheme, self._is_farfield_formulation)
    ...
    def self_consistent_update(self, strain_mf, strain_old_mf, stress_mf,
        ...
        if self._strain_formulation == 'infinitesimal':
            ...
            def_gradient_old = mop.get_tensor_from_mf(
                strain_old_mf, self._n_dim, self._comp_order_nsym)
            if self._self_consistent_scheme == 'regression_material_sym':
                ...
                first_piola_stress = mop.get_tensor_from_mf(
                    stress_mf, self._n_dim, self._comp_order_nsym)
                # Compute homogenized alternative (symmetric) stress-strain
                # conjugate pair
                mat_log_strain, stress_conjugate = \
                    conjugate_material_log_strain(def_gradient,
                                                  first_piola_stress)
                # Set homogenized alternative (symmetric) stress-strain
                ...
                stress_mf = mop.get_tensor_mf(stress_conjugate, self._n_dim,
                                              self._comp_order_sym)
                # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                # Build last converged homogenized first Piola-Kirchhoff stress
                # tensor
                first_piola_stress_old = mop.get_tensor_from_mf(
                    stress_old_mf, self._n_dim, self._comp_order_nsym)
                # Compute last converged homogenized alternative (symmetric)
                # stress-strain conjugate pair
                mat_log_strain_old, stress_conjugate_old = \
                    conjugate_material_log_strain(def_gradient_old,
                                                  first_piola_stress_old)
                # Set last converged homogenized alternative (symmetric)
                # stress-strain conjugate pair (matricial form)
                strain_old_mf = mop.get_tensor_mf(
                    mat_log_strain_old, self._n_dim, self._comp_order_sym)
                stress_old_mf = mop.get_tensor_mf(
                    stress_conjugate_old, self._n_dim, self._comp_order_sym)
                # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                # Compute incremental homogenized material logarithmic strain
                # tensor
                inc_strain_mf = strain_mf - strain_old_mf
                # Compute incremental homogenized stress conjugate to material
                ...
                # Build incremental homogenized deformation gradient tensor
                # (matricial form)
                inc_strain_mf = mop.get_tensor_mf(
                    inc_def_gradient, self._n_dim, self._comp_order_nsym)
                # Compute incremental homogenized first Piola-Kirchhoff stress
                # tensor
                inc_stress_mf = stress_mf - stress_old_mf
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if self._self_consistent_scheme in ('regression',
            ...
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                if self._self_consistent_scheme == 'regression_material_sym':
                    ...
                    #
                    # '1' - The CRVE incremental effective tangent modulus,
                    #       which accounts for a single contraction between the
                    #       CRVE effective tangent modulus and the last
                    #       converged incremental homogenized strain tensor, is
                    #       replaced by the single contraction between the
                    #       reference material elastic tangent modulus and the
                    #       last converged incremental homogenized strain
                    #       tensor.
                    #
                    # '2' - The CRVE incremental effective tangent modulus,
                    #       which accounts for a single contraction between the
                    #       CRVE effective tangent modulus and the last
                    #       converged incremental homogenized strain tensor, is
                    #       replaced by the reference material elastic tangent
                    #       modulus (disregarding or embedding the last
                    ...
                    if option == '1':
                        # Initialize elastic reference material properties
                        # optimizer
                        ref_optimizer = FiniteRegressionSCS(
                            self._strain_formulation, self._problem_type,
                            copy.deepcopy(self._material_properties_old),
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Compute elastic reference material properties based on the parametric
        # optimization of a loss function compatible with a self-consistent
        # scheme
        elif self._self_consistent_scheme == 'self_consistent_optimization':
            # Initialize elastic reference material properties optimizer
            ref_optimizer = SelfConsistentOptimization(
                self._strain_formulation, self._problem_type,
                copy.deepcopy(self._material_properties_old),
                copy.deepcopy(self._material_properties_init),
                copy.deepcopy(self._material_properties_scs_init),
                strain_old_mf, copy.deepcopy(strain_mf), inc_strain_mf,
                inc_stress_mf, eff_tangent_mf=copy.deepcopy(eff_tangent_mf))
            # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            # Compute elastic reference material properties
            E, v = ref_optimizer.compute_reference_properties()
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# The following classes are attempts to develop a reference material properties
# optimizer under finite strains compatible with multiplicative kinematics
# =============================================================================
class FiniteRegressionSCS(ReferenceMaterialOptimizer):
    """Finite strains regression-based self-consistent scheme.

    The CRVE incremental effective tangent modulus, which accounts for a single
    contraction between the CRVE effective tangent modulus and the last
    converged incremental homogenized strain tensor, is replaced by the single
    contraction between the reference material elastic tangent modulus and the
    last converged incremental homogenized strain tensor.
    ...
    """
    def __init__(self, strain_formulation, problem_type,
                 material_properties_old, strain_old_mf, inc_strain_mf,
                 inc_stress_mf):
        """Constructor.
    ...
    def compute_reference_properties(self):
        ...
        # side
        scs_rhs[0] = np.trace(inc_stress)
        scs_rhs[1] = top.ddot22_1(inc_stress,
                                  np.matmul(inc_strain, strain_old))
        # Compute self-consistent scheme system of linear equations coefficient
        # matrix
        scs_matrix[0, 0] = np.trace(top.ddot42_1(
            top.dot42_1(fodiagtrace, strain_old), inc_strain))
        scs_matrix[0, 1] = 2.0*np.trace(top.ddot42_1(
            top.dot42_1(foid, strain_old), inc_strain))
        scs_matrix[1, 0] = top.ddot22_1(top.ddot42_1(
            top.dot42_1(fodiagtrace, strain_old),inc_strain),
            np.matmul(inc_strain, strain_old))
        scs_matrix[1, 1] = 2.0*top.ddot22_1(top.ddot42_1(
            top.dot42_1(foid, strain_old), inc_strain),
            np.matmul(inc_strain, strain_old))
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Solve self-consistent scheme system of linear equations
...
class SelfConsistentOptimization(ReferenceMaterialOptimizer):
    """Parametric optimization of a loss function compatible with a
    self-consistent scheme.

    Two types of self-consistent loss functions are available:

    * 'componentwise_regression' - Loss function essentially defined as the \
                                   componentwise counterpart of the \
                                   regression-based self-consistent scheme. \
                                   The loss if built from the difference \
                                   between the reference material incremental \
                                   homogenized stress tensor and the actual \
                                   material incremental homogenized stress \
                                   tensor.

    * 'consistent_tangent' - Loss function essentially defined as the \
                             componentwise difference between the CRVE \
                             effective tangent modulus and the reference \
    ...
    """
    def __init__(self, strain_formulation, problem_type,
                 material_properties_old, material_properties_init,
                 material_properties_scs_init, strain_old_mf, strain_mf,
                 inc_strain_mf, inc_stress_mf, eff_tangent_mf=None):
        """Elastic reference material properties optimizer constructor.
        ...
        eff_tangent_mf : numpy.ndarray (2d), default=None
            ...
            Only required for 'consistent_tangent' self-consistent loss
            function.
        """
        self._strain_formulation = strain_formulation
        self._problem_type = problem_type
        self._material_properties_old = copy.deepcopy(material_properties_old)
        self._material_properties_init = \
            copy.deepcopy(material_properties_init)
        self._material_properties_scs_init = \
            copy.deepcopy(material_properties_scs_init)
        self._strain_old_mf = copy.deepcopy(strain_old_mf)
    ...
    def compute_reference_properties(
            self, loss_type='componentwise_regression', is_weights=False):
        """Compute elastic reference material properties.

        Parameters
        ----------
        loss_type : {'componentwise_regression', 'consistent_tangent'}, \
                    default='componentwise_regression'
            Type of self-consistent loss function.
        is_weights : bool, default=False
            True if weights are to be considered in the self-consistent loss
        ...
        E_old = self._material_properties_old['E']
        v_old = self._material_properties_old['v']
        # Set optimization parameters initial guess as the last loading
        # increment converged values
        init_shot = {'E': E_old, 'v': v_old}
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Set optimization weights
        if is_weights:
            if loss_type == 'componentwise_regression':
                # Build homogenized strain tensor to compute optimization
                # weights
                if self._strain_formulation == 'infinitesimal':
                    # Build incremental homogenized infinitesimal strain tensor
                    weights_strain = mop.get_tensor_from_mf(
                        self._inc_strain_mf, self._n_dim, self._comp_order_sym)
                else:
                    # Build homogenized deformation gradient tensor
                    def_gradient = mop.get_tensor_from_mf(
                        self._strain_mf, self._n_dim, self._comp_order_nsym)
                    # Compute material logarithmic strain tensor
                    mat_log_strain = compute_material_log_strain(def_gradient)
                    # Build last converged homogenized deformation gradient
                    # tensor
                    def_gradient_old = mop.get_tensor_from_mf(
                        self._strain_old_mf, self._n_dim,
                        self._comp_order_nsym)
                    # Compute last converged material logarithmic strain tensor
                    mat_log_strain_old = \
                        compute_material_log_strain(def_gradient_old)
                ...
                raise RuntimeError('Unknown type of self-consistent loss '
                                   'function.')
        else:
            weights = None
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if loss_type == 'componentwise_regression':
            optimization_function = SelfConsistentCompRegressionLoss(
                self._strain_formulation, self._problem_type,
                copy.deepcopy(self._strain_old_mf),
                copy.deepcopy(self._inc_strain_mf),
                copy.deepcopy(self._inc_stress_mf), lower_bounds, upper_bounds,
                init_shot=init_shot, weights=weights)
        elif loss_type == 'consistent_tangent':
            optimization_function = SelfConsistentTangentLoss(
                self._strain_formulation, self._problem_type,
                copy.deepcopy(self._eff_tangent_mf), lower_bounds,
    ...
    def get_opt_strain_weights(self, strain):
        """Get strain components weights based on the magnitude of normal and
        shear components.

        Parameters
        ----------
        strain : numpy.ndarray (2d)
            Strain tensor: Infinitesimal strain tensor (infinitesimal strains)
            or Material logarithmic strain tensor (finite strains).
        ...
        weights = [normal_avg/norm_factor if comp[0] == comp[1]
                   else shear_avg/norm_factor for comp in comp_order]
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Return
        return weights
# -----------------------------------------------------------------------------
class SelfConsistentTangentLoss(OptimizationFunction):
    """Loss function based on direct self-consistent optimization of reference
    material elastic consistent tangent.

    Optimization loss function is built from the difference between the CRVE
    effective tangent modulus and the reference material elastic tangent
    modulus.
    ...
    _comp_order_nsym : list[str]
        Strain/Stress components nonsymmetric order.
    _parameters_names : tuple[str]
        Optimization parameters names (str).
    _opt_comps : tuple[str]
        Components of the reference material elastic tangent modulus to be
        optimized.
    _norm_bounds : tuple[float]
        Normalization bounds (lower, upper) used to perform the normalization
        of the optimization parameters.
    """
    def __init__(self, strain_formulation, problem_type, eff_tangent_mf,
                 lower_bounds, upper_bounds, init_shot=None, weights=None):
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Set optimization data points associated with the components of the
        # reference material elastic tangent modulus
        if self._strain_formulation == 'infinitesimal':
            if self._problem_type == 1:
                self._opt_comps = ('1111', '2222', '1212', '1122')
            else:
                self._opt_comps = ('1111', '2222', '3333', '1212', '2323',
                                   '1313', '1122', '2233', '1133')
        else:
            if self._problem_type == 1:
                self._opt_comps = ('1111', '2121', '1212', '2222', '1122')
            else:
                self._opt_comps = ('1111', '2121', '3131', '1212', '2222',
                                   '3232', '1313', '2323', '3333', '1122',
                                   '2233', '1133')
        # Check weights attributed to each data point
        if not self._weights is None:
            if len(weights) != len(self._opt_comps):
                raise RuntimeError('Number of weights must be equal to number '
                                   'of data points (reference material '
                                   'elastic tangent modulus components).')
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    def opt_function(self, parameters):
        ...
        try:
            ...
            raise KeyError('Missing optimization function parameter: '
                           + str(err.args[0]))
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Compute Lamé parameters
        ...
        for i, opt_comp in enumerate(self._opt_comps):
            ...
            so_idx = (comp_order.index(opt_comp[:2]),
                      comp_order.index(opt_comp[2:]))
...
class SelfConsistentCompRegressionLoss(OptimizationFunction):
    """Self-consistent loss function based on componentwise regression-based
    scheme.

    Optimization loss function is built from the difference between the
    reference material incremental homogenized stress tensor and the actual
    ...
    _parameters_names : tuple[str]
        Optimization parameters names (str).
    _opt_comps : tuple[str]
        Components of the reference material incremental homogenized stress
        tensor to be optimized.
    _norm_bounds : tuple[float]
        Normalization bounds (lower, upper) used to perform the normalization
        of the optimization parameters.
    """
    def __init__(self, strain_formulation, problem_type, strain_old_mf,
                 inc_strain_mf, inc_stress_mf, lower_bounds, upper_bounds,
                 init_shot=None, weights=None):
        """Optimization function constructor.
        ...
        strain_old_mf : numpy.ndarray (1d)
            Last converged homogenized strain (matricial form).
        inc_strain_mf : numpy.ndarray (2d)
        ...
        inc_stress_mf : numpy.ndarray (2d)
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Set optimization data points associated with the components of the
        # reference material incremental homogenized stress tensor
        if self._strain_formulation == 'infinitesimal':
        ...
        if not self._weights is None:
            if len(weights) != len(self._opt_comps):
                raise RuntimeError('Number of weights must be equal to number '
                                   'of data points (reference material '
                                   'elastic incremental homogenized stress '
                                   'tensor components).')
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Initialize optimization parameters normalization bounds
        self.set_norm_bounds()
    ...
    def opt_function(self, parameters):
        ...
        if self._strain_formulation == 'finite':
            # Set finite strains formulation option:
            # '1' - The CRVE incremental effective tangent modulus, which
            #       accounts for a single contraction between the CRVE
            #       effective tangent modulus and the last converged
            #       incremental homogenized strain tensor, is replaced by the
            #       the single contraction between the reference material
            #       elastic tangent modulus and the last converged incremental
            #       homogenized strain tensor.
            # '2' - The CRVE incremental effective tangent modulus, which
            #       accounts for a single contraction between the CRVE
            #       effective tangent modulus and the last converged
            #       incremental homogenized strain tensor, is replaced by the
            #       reference material elastic tangent modulus (disregarding or
            #       embedding the last converged incremental homogenized strain
            #       tensor).
            option = '1'
            if option == '1':
                # Get last converged strain tensor
                strain_old = mop.get_tensor_from_mf(
                    self._strain_old_mf, self._n_dim, comp_order)
                # Set reference material incremental tangent modulus (matricial
                # form)
                inc_elastic_tangent_mf = mop.get_tensor_mf(
                    top.dot42_1(elastic_tangent, strain_old), self._n_dim,
                                comp_order)
            else:
                # Set reference material incremental tangent modulus (matricial
                # form)
                inc_elastic_tangent_mf = mop.get_tensor_mf(
                    elastic_tangent, self._n_dim, comp_order)
        else:
            # Set reference material incremental tangent modulus (matricial
            # form)
        ...
        inc_stress_0_mf = np.matmul(inc_elastic_tangent_mf,
                                    self._inc_strain_mf)
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Initialize parametric and reference solution
        y = np.zeros(len(self._opt_comps))
