...
def refelastictanmod(problem_dict, mat_prop_ref):
    ...
    _, foid, _, fosym, fodiagtrace, _, _ = top.get_id_operators(n_dim)
    ...
    if self_consistent_scheme == 1:
        ...
        # OPTION 2 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            ...
            if problem_type == 1:
                ...
                inc_strain_mf = top.get_state_3Dmf_from_2Dmf(problem_type, inc_strain_mf,
                                                             comp_order=0.0)
                inc_stress_mf = top.get_state_3Dmf_from_2Dmf(problem_type, inc_stress_mf,
                                                             inc_stress_33)
