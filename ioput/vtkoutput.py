...
class VTKOutput:
    ...
    '''
    # --------------------------------------------------------------------------------------
    def __init__(self, type, version, byte_order, format, precision, header_type, base_name,
                 vtk_dir, pvd_dir=None):
        '''VTK output constructor.
        ...
        format : str
            VTK file format.
        precision : str, {'SinglePrecision', 'DoublePrecision'}
    ...
    def write_VTK_file_clustering(self, crve):
        ...
        xml = XMLGenerator(self._type, self._version, self._byte_order, self._format,
                           self._precision, self._header_type)
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Get RVE dimensions
        ...
        voxels_clusters_flat = self._reset_labels_from_zero(voxels_clusters.flatten('F'))
    ...
    def write_VTK_file_time_step(self, time_step, problem_type, crve, clusters_state,
        ...
        min_val = min(data_list)
        max_val = max(data_list)
        data_parameters = {'Name': 'Von Mises Eq. Stress', 'format': self._format,
                           'RangeMin': min_val, 'RangeMax': max_val}
        xml.write_cell_data_array(vtk_file, data_list, data_parameters)
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        min_val = min(data_list)
        max_val = max(data_list)
        data_parameters = {'Name': 'Von Mises Eq. Strain', 'format': self._format,
                           'RangeMin': min_val, 'RangeMax': max_val}
        xml.write_cell_data_array(vtk_file, data_list, data_parameters)
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    # --------------------------------------------------------------------------------------
    def _set_cell_data_name(self, comp_order_sym, comp_order_nsym, var_name, var_type,
                            idx=None, model_name=None):
        ...
        model_name : str, default=None
            Material constitutive model name. If provided, the model name is included as a
            prefix in the variable cell data array name.
        '''
        ...
        return data_name
    # --------------------------------------------------------------------------------------
    def _set_state_var_descriptors(self, n_dim, comp_order_sym, comp_order_nsym, var_name,
                                   source, model_state_variables=None, cluster=None,
                                   clusters_state=None):
        '''Set state variable descriptors required to output associated cell data array.
        ...
        var_name : str
            State variable name.
        source : src, {'model_state_variables', 'clusters_state'}
            Source where state variable is stored.
        model_state_variables : dict, default=None
            Material model state variables (required if source='model_state_variables').
        cluster : int, default=None
            Cluster label (required if source='clusters_state').
        clusters_state : dict, default=None
            Material constitutive model state variables (item, dict) associated to each
            material cluster (key, str) (required if source='clusters_state').

        Returns
        -------
        var : int, float, bool or array
            State variable.
        var_type : str
            State variable type.
        var_n_comp : int
    ...
    # --------------------------------------------------------------------------------------
    def _state_var_cell_data_array(self, n_dim, comp_order_sym, comp_order_nsym,
                                   material_phases, material_phases_models, phase_clusters,
                                   voxels_clusters, clusters_state, var_name,
                                   var_type, comp_idx=None, model_name=None):
        '''Build state variable cell data array.
        ...
        -------
        rg_array : ndarray
            Array of state variable component of shape equal to RVE regular grid
        ...
        for mat_phase in material_phases:
            if model_name is None or \
                ...
                for cluster in phase_clusters[mat_phase]:
                    ...
                    cluster_var, _, _ = self._set_state_var_descriptors(n_dim,
                        comp_order_sym, comp_order_nsym, var_name,
                    ...
                    if var_type in ['int', 'bool', 'float']:
                        ...
                        rg_array = np.where(rg_array == str(cluster), cluster_var[comp_idx],
                                            rg_array)
                    elif var_type == 'sym_matrix_mf':
                        idx = tuple([int(x) - 1 for x in comp_order_sym[comp_idx]])
                        rg_array = np.where(rg_array == str(cluster), cluster_var[idx],
                                            rg_array)
                    elif var_type == 'nsym_matrix_mf':
                        idx = tuple([int(x) - 1 for x in comp_order_nsym[comp_idx]])
                        rg_array = np.where(rg_array == str(cluster), cluster_var[idx],
                                            rg_array)
                    else:
                        idx = tuple([int(x) - 1 for x in comp_order_nsym[comp_idx]])
        ...
        if any(isinstance(x, str) for x in list(rg_array.flatten('F'))):
            raise RuntimeError('Incomplete VTK output cell data array - ' + var_name + '.')
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Return
        return rg_array
    # --------------------------------------------------------------------------------------
...
class VTKCollection:
    ...
    def init_vtk_collection_file(self):
        ...
        vtk_file.write('<' + 'VTKFile type=' + XMLGenerator.enclose('Collection') + ' ' + \
...
class XMLGenerator:
    ...
    '''
    # --------------------------------------------------------------------------------------
    def __init__(self, type, version, byte_order, format, precision, header_type):
    ...
    def write_file_header(self, vtk_file):
        ...
        vtk_file.write('<' + 'VTKFile type=' + XMLGenerator.enclose(self._type) + ' ' + \
                       ...
                       'byte_order=' + XMLGenerator.enclose(self._byte_order) + ' ' + \
                       'header_type=' + XMLGenerator.enclose(self._header_type) + '>' + \
                       '\n')
    # --------------------------------------------------------------------------------------
    ...
    def write_open_dataset_elem(self, vtk_file, dataset_parameters):
        ...
        vtk_file.write(self._indent + '<' + self._type + ' ' + \
                       ' '.join([ key + '=' + XMLGenerator.enclose(parameters[key]) \
    ...
    def enclose(x):
        ...
        x : string convertible
            Input to be converted into a string enclosed in literal quotation marks.
